...
class PlayerData: public LivingThingData
{
public:
	PlayerData(void);
	~PlayerData(void);
	...
	CC_SYNTHESIZE(cocos2d::CCPoint, rolePos, RolePos);


	WX_SYNTHESIZE(int, entryid, id);
	WX_SYNTHESIZE(int, tx, Tx);
	WX_SYNTHESIZE(int, ty, Ty);
	WX_SYNTHESIZE(std::string, name, Name);
	WX_SYNTHESIZE(float, moveSpeed, MoveSpeed);
	WX_SYNTHESIZE(int, gender, Gender);
	WX_SYNTHESIZE(int, level, Level);
	WX_SYNTHESIZE(int, kind, Kind);
	WX_SYNTHESIZE(int, currentHP, CurrentHP);
	WX_SYNTHESIZE(int, currentMP, CurrentMP);
	WX_SYNTHESIZE(int, totalHP, TotalHP);
	WX_SYNTHESIZE(int, totalMP, TotalMP);
	WX_SYNTHESIZE(int, faction, Faction);
	WX_SYNTHESIZE(int, mapId, MapId);
protected:
